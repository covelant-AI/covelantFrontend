...
export default function OpponentSelector({
  ...
  const [selectedState, setSelectedState] = useState<Player | null>(selected || null)
  const [showCreateForm, setShowCreateForm] = useState(false)
  const [newFirstName, setNewFirstName] = useState('')
  const [newLastName, setNewLastName] = useState('')
  const containerRef = useRef<HTMLDivElement>(null)

  // Sync internal state with prop on first load or change
  useEffect(() => {
      ...
      <div className="flex flex-col items-center w-full">
        {selectedState ? (
          <div className="h-45 w-full flex items-center justify-center text-black font-bold">
            <div className="relative min-w-40 w-full h-full rounded-xl overflow-hidden">
              <Image
                src={selectedState.avatar || "/images/default-avatar.png"}
                alt="User"
                fill
                style={{ objectFit: "cover" }}
              />
            </div>
            <div className="absolute bottom-125 md:bottom-130 max-[360px]:bottom-137 flex items-center text-black bg-gray-400 bg-opacity-50 rounded px-2">
              <button
