...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    email: string; id: number; firstName: string | null; lastName: string | null; avatar: string | null; coaches: number | null; }[] = [];
    ...
    if (coach) {
      playersWithStats = await Promise.all(
        coach.players.map(async (player) => {
          ...
          return { ...player, stats, coaches: (player as any).coaches ?? null };
        })
      );
